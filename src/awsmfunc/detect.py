...
def _detect(
    ...
    if filter_consecutives:
        detections_list = merge_detections(
            detections_list,
...
def get_min_diff_consecutives(
    input_arg: Union[str, PathLike, List[int]], cycle: int = 1, tolerance: int = 0
) -> List[int]:
    import numpy as np
...
def merge_detections(
    input_arg: Union[str, PathLike, List[int]],
...
def banddtct(
    ...
    def detect_func(detections: Set[int]):
        def banding_detect(n, f, clip, detections, hi, lo, diff, check_next):
            if f[0].props.PlaneStatsAverage >= lo and f[0].props.PlaneStatsAverage <= hi:
                if check_next:
                    ...
                    if (
                        f[1].props.PlaneStatsDiff < diff
                        and f[2].props.PlaneStatsAverage >= lo / 2
                        and f[2].props.PlaneStatsAverage <= hi
                    ):
        ...
        return core.std.FrameEval(
            clip,
            partial(banding_detect, clip=clip, detections=detections, hi=hi, lo=lo, diff=diff, check_next=check_next),
            prop_src=prop_src,
        )
...
def cambidtct(
    ...
    def detect_func(detections):
        ...
        return core.std.FrameEval(
            clip,
            partial(banding_detect, clip=clip, detections=detections, diff=diff, check_next=check_next),
            prop_src=prop_src,
        )
    ...
    def detect_func(detections):
        ...
        for _ in column_list:
            for i in _:
                ...
                processed = core.std.FrameEval(
                    clip, partial(line_detect, clip=clip, detections=detections, thr=thr), prop_src=clip_diff
                )
        for _ in row_list:
            for i in _:
...
def brdrdtct(
    ...
    clip = clip.acrop.CropValues(
        range=ac_range, left=left, right=right, top=top, bottom=bottom, color=color, color_second=color_second
    )

    def detect_func(detections):
        def border_detect(n, f, clip, detections):
            if (f.props.CropTopValue > 0 and f.props.CropBottomValue > 0) or (
                f.props.CropLeftValue > 0 and f.props.CropRightValue > 0
            ):
                detections.add(n * cycle)
...
class SceneChangeDetector(str, Enum):
    ...
    def run_detection(
        self, clip: vs.VideoNode, av_sc_cli: Optional[str] = None, output: Optional[Union[str, PathLike]] = None
    ) -> List[int]:
...
def measure_hdr10_content_light_level(
    clip: vs.VideoNode,
