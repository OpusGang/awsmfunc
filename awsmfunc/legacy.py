...
def autogma(clip: vs.VideoNode, adj: float = 1.3, thr: float = 0.40) -> vs.VideoNode:
    ...
    def hilo(n: int, f: vs.VideoFrame) -> vs.VideoNode:
        g = core.std.Levels(luma, gamma=adj)

        if f.props.PlaneStatsAverage > thr:
            return g.std.Invert().sub.Subtitle(f"Current average: {f.props.PlaneStatsAverage}")
        else:
            return g.sub.Subtitle(f"Current average: {f.props.PlaneStatsAverage}")

    prc = core.std.FrameEval(luma, hilo, prop_src=s)
